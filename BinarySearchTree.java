...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    // check if parent and child are related
    // first check if either the child or parent is null
    if (child == null || parent == null)
      throw new IllegalArgumentException();

    // Second, check if the child is related to the parent
    if ((child.up == null || !child.up.equals(parent)))
      throw new IllegalArgumentException();

    // Third, check if the parent is related to the child
    if ((parent.down[0] == null || !parent.down[0].equals(child))
        && (parent.down[1] == null || !parent.down[1].equals(child)))
      throw new IllegalArgumentException();

    Node<T> grandparent = parent.up; // keep a pointer to parent's parent node to re-wire rotated
                                     // nodes later
    boolean isRight = parent.isRightChild(); // remember if the parent is a right child since we are
                                             // changing the parent's up pointer
    // ROTATE LEFT
    if (child.isRightChild()) {
      Node<T> childLeftSubTree = child.down[0];
      child.down[0] = parent; // make parent child's left child
      parent.up = child;
      parent.down[1] = childLeftSubTree; // put child's left subtree to parent's right side
      if (childLeftSubTree != null)
        childLeftSubTree.up = parent;

      // ROTATE RIGHT
    } else {
      Node<T> childRightSubTree = child.down[1];
      child.down[1] = parent; // make parent child's right child
      parent.up = child;
      parent.down[0] = childRightSubTree; // put child's right subtree to parent's left side
      if (childRightSubTree != null)
        childRightSubTree.up = parent;
    }

    // fix root if the root changes after rotation
    if (grandparent == null){
      root = child;
      root.up = null;
    }
    // if root doesn't need to be changed, rotated nodes need to be linked to the grandparent
    else {
      child.up = grandparent;
      // make rotated subtree either the left or right subtree of the grandparent
      if (isRight)
